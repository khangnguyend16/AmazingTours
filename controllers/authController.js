...
const createSendToken = (user, statusCode, res) => {
    ...
    // đảm bảo cookie chỉ được gửi qua HTTPS, giúp bảo mật hơn
...
exports.protect = catchAsync(async (req, res, next) => {
    ...
    req.user = freshUser; // Middleware hoặc route tiếp theo có thể truy cập req.user
    next();
})


exports.restrictTo = (...roles) => {  //creating an array of all arguments
    return (req, res, next) => {      // return a middleware function
        // roles ['admin','lead-guide']. 
        if (!roles.includes(req.user.role)) {  //req.user comes from protect middleware
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
    ...
    await user.save({ validateBeforeSave: false });  // Lưu token đã băm vào database
...
exports.resetPassword = catchAsync(async (req, res, next) => {
    ...
    user.passwordResetToken = undefined;
    user.passwordResetExpires = undefined;
    await user.save();  // We want the validator to confirm if password == passwordConfirm
